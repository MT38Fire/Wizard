...
function AccessRoomScreen({ username, onLogout, onAccess, onRoom }) {
  ...
  const accessOptions = [
    ...
    { id: 3, label: 'Crea Stanza', color: '#FF9800' },
    { id: 4, label: 'Profilo', color: '#9C27B0', isImp: true }
  ];
  ...
  const handleButtonClick = async (option) => {
    if (option.id === 1) {
      ...
      const roomName = prompt("Inserisci il nome della stanza:");
      const maxPlayers = prompt("Inserisci il numero massimo di giocatori (3-5):");
  ...
  const handleJoinRoom = async () => {
    //if (!selectedRoom) return; // Non dovrebbe poter succedere
    if (joinDisabled) return; // Evita doppio click
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ color: '#333', marginBottom: '10px' }}>Benvenuto, {username}!</h1>
        <p style={{ color: '#666', marginBottom: '30px' }}>Scegli dove vuoi andare</p>
        ...
        {/* Popup impostazioni sostituito con Popup profilo */}
        {showProfile && (
          <div style={{
            position: 'fixed',
            top: '50%',
            ...
            <h3 style={{ color: '#9C27B0', marginTop: '5px', marginBottom: '20px' }}>
              Profilo di {username}
            </h3>
            ...
            {userProfile ? (
              <div style={{ textAlign: 'left', marginBottom: '20px' }}>
                ...
                <p><strong>Data registrazione:</strong> {new Date(userProfile.registration_date).toLocaleDateString()}</p>
                <p><strong>Partite giocate:</strong> {userProfile.games_played}</p>
                <p><strong>Vittorie:</strong> {userProfile.wins}</p>
                <p><strong>Punteggio totale:</strong> {userProfile.total_points}</p>
                
                <h4 style={{ marginTop: '20px', color: '#9C27B0' }}>Storico partite:</h4>
                  ...
                  <ul style={{ listStyle: 'none', padding: 0 }}>
                    {matchHistory.map((match, index) => (
                      <li key={index} style={{ 
                        ...
                        <p><strong>Stanza:</strong> {match.room_name}</p>
                        <p><strong>Data:</strong> {new Date(match.match_date).toLocaleString()}</p>
                        <p><strong>Punteggio:</strong> {match.points}</p>
                        <p><strong>Posizione:</strong> {match.position}°</p>
                      </li>
                    ))}
                  </ul>
                ) : (
...
export default AccessRoomScreen;
